...
const gameElementsContainer = document.querySelector('.game-elements-container');
const burgerMenuListItemsCollection = Array.from(document.querySelector('.burger-menu-list').children);
...
export function movingToCategory(event, categoryObjectsArray) {
    ...
    categoryName = event.target.closest('.category-menu-item').firstChild.innerText.toLowerCase();
    hideBurgerMenu();
...
function modifyInterfaceForTrainMode(wordCardsCollection) {
    wordCardsCollection.forEach(card => card.classList.add('word-card-in-train-mode'));
    wordCardsCollection.forEach(card => card.classList.remove('word-card-in-play-mode', 'unplayed-word', 'played-card'));
...
function modifyInterfaceForPlayMode(wordCardsCollection) {
    wordCardsCollection.forEach(card => card.classList.add('word-card-in-play-mode', 'unplayed-word'));
    wordCardsCollection.forEach(card => card.classList.remove('word-card-in-train-mode'));
    bigPlayButtonContainer.classList.add('play-button-container-visible');
    mainContainer.classList.add('main-container-play-mode');
}
...
export function switchMode() {
    const wordCardsCollection = Array.from(document.querySelectorAll('.word-card'));
    ...
    if (isPlayModeChecked()) {
        setPlayMode(wordCardsCollection, playBtnsCollection, bigPlayButtonContainer);
    } else {
        setTrainMode(wordCardsCollection, playBtnsCollection, bigPlayButtonContainer);
    }
}
...
export function createOrUpdateLocalStorage(word, trained, correct, incorrect) {
    if (!localStorage[word]) {
        const storageItem = createObjectForLocalStorage(trained, correct, incorrect);
...
export function startGame() {
    const playingCardsCollection = document.getElementsByClassName('unplayed-word');
...
const menuItem = event.target.closest('.brgr-menu-item');
