...
export class Game {
    ...
    constructor(wordCardsCollection) {
        ...
        this.playingCard;
        this.attemtsToWin = this.wordCardsCollection.length;
        this.attemts = 0;
    ...
    startGame() {
        ...
        this.mainContainer.addEventListener('click', this.clickHanlerForCheckAnswer);

        document.body.addEventListener('click', (event) => { 
            if (event.target.closest('.switch') || event.target.closest('.brgr-menu-item')) {
                this.stopGame();
            }
        }), { once: true };
     ...
     */
    clickHanlerForCheckAnswer = event => this.checkAnswer(event);
    ...
    gameOver() {
        setTimeout(() => {
            ...
            if (this.attemts > this.attemtsToWin) {
                const mistakesNumElem = document.querySelector('.mistakes-number');
                const mistakesNum = this.attemts - this.attemtsToWin;
                console.log(mistakesNum, this.attemts, this.attemtsToWin)
    ...
    changePlayButton(state) {
        if (state === 'gameStarted') {
            ...
            this.bigPlayButton.addEventListener('click', this.repeatButtonClickHandler);
        } else {
            this.bigPlayButton.classList.remove('play-button-to-repeat');
            this.bigPlayButton.innerHTML = 'play';

            this.bigPlayButton.removeEventListener('click', this.repeatButtonClickHandler);
        }
    }
     ...
     */
    stopGame() {
        this.mainContainer.removeEventListener('click', this.clickHanlerForCheckAnswer);
        this.playingCard;
        this.badgesContainer.innerHTML = '';
        this.badgesCollection = [];
        this.attemts = 0;
        this.changePlayButton('gameStoped');
